...
export default function RdvPasVenusPage() {
  ...
  const generateDailyData = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const pasVenus = totalRdv > 0 ? Math.floor(random * totalRdv) : 0;
      const tauxPasVenus = totalRdv > 0 ? ((pasVenus / totalRdv) * 100).toFixed(1) : '0';
      dailyData.push({
        date: `${day.toString().padStart(2, '0')}/${(selectedMonth + 1).toString().padStart(2, '0')}`,
        totalRdv,
        pasVenus,
        tauxPasVenus: tauxPasVenus + '%'
      });
    }
    return dailyData;
  };
      ...
      <div className="grid grid-cols-3 gap-4 mb-8 animate-fadeIn">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6  transition-all group">
          ...
          <div>
            ...
            <p className="text-3xl font-light text-gray-900">{totalTauxPasVenus}%</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">TAUX EN LIGNE</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {dailyData.map((day, index) => (
                <tr key={index} className={`hover:bg-gray-50 transition-colors ${day.totalRdv === 0 ? 'bg-gray-50/50' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`text-sm font-medium ${day.pasVenus > 0 ? 'text-emerald-600' : 'text-gray-900'}`}>{day.pasVenus}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
