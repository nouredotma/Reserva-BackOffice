...
export default function DailyStatisticsPage() {
  ...
  const generateDailyData = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const totalOffers = hasActivity ? Math.floor(Math.random() * 5) : 0;
      const inSalon = hasActivity ? Math.floor(Math.random() * (totalOffers + 1)) : 0;
      const online = totalOffers - inSalon;
      const onlineRate = totalOffers > 0 ? ((online / totalOffers) * 100).toFixed(2) : '0';
  ...
  const totals = dailyData.reduce((acc, day) => ({
    totalOffers: acc.totalOffers + day.totalOffers,
    inSalon: acc.inSalon + day.inSalon,
    online: acc.online + day.online
  }), { totalOffers: 0, inSalon: 0, online: 0 });

  const totalOnlineRate = totals.totalOffers > 0
    ? ((totals.online / totals.totalOffers) * 100).toFixed(1)
    : '0';
    ...
    <div className="min-h-screen">
      ...
      <div className="mb-12 pt-20 animate-slideUp">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-5xl font-light text-gray-900 tracking-tight mb-2">Appointments</h1>
            <p className="text-sm text-gray-400">Detailed view by date</p>
          </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8 animate-fadeIn">
        <div className="bg-white rounded-xl border border-gray-100 p-6  transition-all group">
          ...
          <div>
            ...
            <p className="text-3xl font-light text-gray-900">{totals.totalOffers}</p>
          </div>
        </div>
              ...
              {dailyData.map((day, index) => (
                <tr
                  ...
                  className={`hover:bg-gray-50 transition-colors ${
                    day.totalOffers === 0 ? 'bg-gray-50/50' : ''
                  }`}
                >
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className="text-sm font-medium text-gray-900">{day.totalOffers}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
